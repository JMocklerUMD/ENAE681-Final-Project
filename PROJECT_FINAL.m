%% INTRODUCTION
% JMockler for ENAE681 - Engineering Optimization
% Due 12 DEC23
% Final Project
...
opts = optimoptions("fmincon"); opts.Display = "iter";
opts.Algorithm = "interior-point"; opts.MaxFunctionEvaluations = numWayPoints*500;

% set initial conditions
xWayPoints = linspace(0,sizeX,numWayPoints+2)';
yWayPoints = sizeY/2 * ones(numWayPoints+2,1);
ipt = yWayPoints(2:end-1)'; ipt = ipt(:); %int cnds
...
plot( xWayPoints,yWayPoints,"Color","#0072BD","LineStyle","none","Marker",".","MarkerSize",16);

pathPoints = waypointsToPath([xWayPoints,yWayPoints],method,sizeX,sizeY,integrationFineness);
plot(pathPoints(:,1),pathPoints(:,2),"color","#0072BD","LineWidth",2);

fmincon_int_pt_time = getTimeFromPath(pathPoints, dHx, dHy,V0, numWayPoints, Heights);
fmincon_int_pt_improve = straightLineTime - fmincon_int_pt_time;
...
opts = optimoptions("fmincon"); opts.Display = "iter";
opts.Algorithm = "sqp"; opts.MaxFunctionEvaluations = numWayPoints*500;
...
xWayPoints = optimalWayPoints(:,1);
yWayPoints = optimalWayPoints(:,2);
plot( xWayPoints,yWayPoints,"Color","#D95319","LineStyle","none","Marker",".","MarkerSize",16);

pathPoints = waypointsToPath([xWayPoints,yWayPoints],method,sizeX,sizeY,integrationFineness);
plot( pathPoints(:,1),pathPoints(:,2),"Color","#D95319","LineWidth",2);

fmincon_spq_time = getTimeFromPath(pathPoints, dHx, dHy,V0, numWayPoints, Heights);
fmincon_spq_improve = straightLineTime - fmincon_spq_time;


%% OPTIMIZATION - active-set
...
opts = optimoptions("fmincon"); opts.Display = "iter";
opts.Algorithm = "active-set"; opts.MaxFunctionEvaluations = numWayPoints*500;
...
xWayPoints = optimalWayPoints(:,1);
yWayPoints = optimalWayPoints(:,2);
plot(xWayPoints,yWayPoints,"Color","#EDB120","LineStyle","none","Marker",".","MarkerSize",16);

pathPoints = waypointsToPath([xWayPoints,yWayPoints],method,sizeX,sizeY,integrationFineness);
plot( pathPoints(:,1),pathPoints(:,2),"Color","#EDB120","LineWidth",2);

fmincon_active_set_time = getTimeFromPath(pathPoints, dHx, dHy,V0, numWayPoints, Heights);
fmincon_active_set_improve = straightLineTime - fmincon_active_set_time;
...
tic;
optimalWayPoints = fminunc(objectiveFun,ipt(:));
unc_time = toc;
optimalWayPoints = [0 sizeY/2; reshape([xWayPoints(2:end-1)'; optimalWayPoints'],2,[])'; sizeX sizeY/2];
...
fminunc_time = getTimeFromPath(pathPoints, dHx, dHy,V0, numWayPoints, Heights);
fminunc_improve = straightLineTime - fminunc_time;
...
xWayPoints = linspace(0,sizeX,numWayPoints+2)';
yWayPoints = sizeY/2 * ones(numWayPoints+2,1);
ipt = yWayPoints(2:end-1)';
ipt = ipt(:);
lb = zeros(size(ipt(:)))+(sizeY/3);
ub = sizeY.*ones(size(ipt(:)))-(sizeY/3);

tic;
optimalWayPoints = simulannealbnd(objectiveFun,ipt(:), lb, ub);
sim_ann_time = toc;
optimalWayPoints = [0 sizeY/2; reshape([xWayPoints(2:end-1)'; optimalWayPoints'],2,[])'; sizeX sizeY/2];

xWayPoints = optimalWayPoints(:,1);
yWayPoints = optimalWayPoints(:,2);
plot(xWayPoints,yWayPoints,"Color","#A2142F","LineStyle","none","Marker",".","MarkerSize",16);

pathPoints = waypointsToPath([xWayPoints,yWayPoints],method,sizeX,sizeY,integrationFineness);
h_path = plot( pathPoints(:,1),pathPoints(:,2),"Color","#A2142F","LineWidth",2);

sim_time_opt = getTimeFromPath(pathPoints, dHx, dHy,V0, numWayPoints, Heights);
sim_time_improve = straightLineTime - sim_time_opt;

% ADD LEGEND TO PLOT
legend('Location','northeastoutside'); 
legend('Contour','Straight Line', 'Straight Line','Con, Interior Pt', 'Con, Interior Pt', 'Con, SQP',' Con, SQP', 'Con, Active Set','Con, Active Set','Uncon','Uncon', 'Sim. Ann.','Sim. Ann.')
